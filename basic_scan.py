...
def auto_scan(output_dir="Scans", file_format="jpg", quality="medium"):
    """
    ...
    try:
        ...
        try:
            ...
            print(f"⚠️ Could not set some properties: {prop_err}")
        ...
        if file_format.lower() == "pdf":
            ...
            print(f"✅ Scan complete ({quality}, {dpi} DPI). Saved as {pdf_file}")
            return pdf_file
            ...
            print(f"✅ Scan complete ({quality}, {dpi} DPI). Saved as {output_file}")
            return output_file
...
def images_to_pdf(image_files, output_pdf):
    ...
    Args:
        image_files (list): List of image file paths to combine
        output_pdf (str): Output PDF file path
    
    Returns:
        str or None: Path to created PDF on success, None on failure
    ...
    if not image_files:
        ...
        print("❌ No images provided for PDF conversion.")
        return None
    ...
    try:
        ...
        logger.info(f"PDF created with {len(image_files)} pages: {output_pdf}")
        print(f"✅ PDF created with {len(image_files)} pages: {output_pdf}")
        return output_pdf
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if file_name1:
        ...
        print(f"✅ Scan completed successfully: {file_name1}")
        ...
        #     print(f"✅ PDF created: {pdf_result}")
    else:
        logger.error("Scan failed. Check scanner connection and try again")
        print("❌ Scan failed. Check scanner connection and try again.")